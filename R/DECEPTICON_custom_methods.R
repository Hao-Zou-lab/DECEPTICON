...
DECEPTICON_epic_custom <- function(mix.mat, signature.matrix){
  ...
  epic_sig_1 = EPIC::EPIC(mix.mat, reference =  list(refProfiles=sig_1,sigGenes = rownames(sig_1)))
  ...
  if(length(signature.matrix) > 1){
    for (i in 2:length(signature.matrix)) {
      ...
      assign(paste0('epic_sig_','',i,sep=''),EPIC::EPIC(mix.mat, reference =  list(refProfiles=get(paste0('sig_','',i,sep='')),sigGenes = rownames(get(paste0('sig_','',i,sep=''))))))
...
DECEPTICON_mcp_custom <- function(data,signature.matrix){
  ...
  mcp_sig_1 = MCPcounter::MCPcounter.estimate(data,featuresType="HUGO_symbols",genes=sig_1)
  ...
  if(length(signature.matrix) > 1){
    ...
    for (i in 2:length(signature.matrix)) {
      ...
      assign(paste0('MCP_sig_','',i,sep=''),MCPcounter::MCPcounter.estimate(data,featuresType="HUGO_symbols",genes=a))
...
DECEPTICON_scdc_custom <- function(data, signature.matrix){
  for (i in 1:length(signature.matrix)) {
    ...
    assign(paste0('scdc_sig_','',i,sep=''), SCDC::SCDC_prop_ONE(bulk.eset = data ,sc.eset = eset.qc$sc.eset.qc,ct.varname = "cellname",sample = "subjects",
...
DECEPTICON_music_custom <- function(data, signature.matrix){
  ...
  for (i in 1:length(signature.matrix)) {
    ...
    assign(paste0('music_sig_','',i,sep=''), music_prop(bulk.mtx = Biobase::exprs(data),sc.sce = eset,clusters = "celltype",samples = "sampleID"))
...
DECEPTICON_batman_custom <- function(signature.matrix){
  for (i in 1:length(signature.matrix)) {
    assign(paste0('batman_sig_','',i,sep=''), batman::batman(runBATMANDir = paste0("./batman/sig_", '', i, sep=""),showPlot = FALSE))
