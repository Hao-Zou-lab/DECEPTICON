...
DECEPTICON_epic <- function(mix.mat){
  ...
  epic_BRef <- EPIC(mix.mat, reference =list(refProfiles=BRef$refProfiles,sigGenes = BRef$sigGenes,refProfiles.var = BRef$refProfiles.var))
  write.table(epic_BRef[['cellFractions']], './res/EPIC_BRef.txt', sep='\t', row.names= T, col.names= T, quote=F)
  epic_TRef <- EPIC(mix.mat, reference =list(refProfiles=TRef$refProfiles,sigGenes = TRef$sigGenes,refProfiles.var = TRef$refProfiles.var))
  ...
  epic_ciber <- EPIC(mix.mat, reference =  list(refProfiles=LM22,sigGenes = rownames(LM22)))
  ...
  epic_quan <- EPIC(mix.mat, reference =  list(refProfiles=quan_base,sigGenes = rownames(quan_base)))
  ...
  epic_immu <- EPIC(mix.mat, reference =  list(refProfiles=immu_base,sigGenes = rownames(immu_base)))
...
DECEPTICON_mcp <- function(data){
  ...
  mcp_ciber = MCPcounter.estimate(data,featuresType="HUGO_symbols",genes=ciber_gene)
  ...
  mcp_TRef = MCPcounter.estimate(data,featuresType="HUGO_symbols",genes=TRef_gene)
  ...
  mcp_BRef = MCPcounter.estimate(data,featuresType="HUGO_symbols",genes=BRef_gene)
  ...
  mcp_quan = MCPcounter.estimate(data,featuresType="HUGO_symbols",genes=quan_gene)
  ...
  mcp_immu = MCPcounter.estimate(data,featuresType="HUGO_symbols",genes=immu_gene)
...
DECEPTICON_scdc <- function(data){
  ...
  for (i in 1:length(signature_matrix)) {
    ...
    assign(res_id[i], SCDC_prop_ONE(bulk.eset = data ,sc.eset = eset.qc$sc.eset.qc,ct.varname = "cellname",sample = "subjects",
...
DECEPTICON_music <- function(data){
  ...
  for (i in 1:length(signature_matrix)) {
    ...
    assign(res_id[i], music_prop(bulk.eset = data,sc.eset = eset,clusters = "celltype",samples = "sampleID"))
  }
...
DECEPTICON_batman <- function(){
  ...
  for (i in 1:length(signature_matrix)) {
    assign(res_id[i], batman(runBATMANDir = signature_matrix[i],showPlot = FALSE))
  }
